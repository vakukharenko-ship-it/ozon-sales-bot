...
async def fetch_finance_accruals_by_day(date_str: str) -> List[Dict]:
    ...
    while True:
        ...
        try:
            ...
            write_log(f"❌ Ошибка загрузки начислений за {date_str} (стр. {page}): {e}")
            break
...
async def fetch_finance_transactions(date_from, date_to, progress_callback=None):
    ...
    while current <= end_dt:
        ...
        if progress_callback and total_days > 0:
            progress = int((day_idx / total_days) * 100)
            await progress_callback(f"Загрузка начислений за {date_str}...", progress)
